...
public class SymbolTable {
    private final Map<String, SymbolTableEntry> symbolList = new HashMap<>();
    ...
    public SymbolTableEntry get(String text) {
        // 在符号表中查找符号对应的条目
        SymbolTableEntry symbol = symbolList.get(text);
        // 如果找不到对应的条目，则抛出异常
        if (symbol == null) {
            throw new RuntimeException("符号在表中不存在: " + text);
        }
        return symbol;
    }
    ...
    public SymbolTableEntry add(String text) {
        if (!has(text)) {
            symbolList.put(text,new SymbolTableEntry(text));
            return new SymbolTableEntry(text);
        } else {
            throw new RuntimeException("符号在表中已存在:" + text);
        }
    }
    ...
    public boolean has(String text) {
        return symbolList.get(text) != null;
    }
